...
export class Camera extends Component {
  ...
  get aspectRatio(): number {
    return this._customAspectRatio ?? (this._width * this._viewport.z) / (this._height * this._viewport.w);
  }
  ...
  get projectionMatrix(): Matrix {
    if (
      ...
      this._lastAspectSize.x === this._width &&
      this._lastAspectSize.y === this._height
    ) {
  ...
  screenToViewportPoint<T extends Vector2 | Vector3>(point: Vector3 | Vector2, out: T): T {
    ...
    out.x = (point.x / this._width - viewport.x) / viewport.z;
    out.y = (point.y / this._height - viewport.y) / viewport.w;
    return out;
  }
  ...
  viewportToScreenPoint<T extends Vector2 | Vector3 | Vector4>(point: T, out: T): T {
    const viewport = this.viewport;
    out.x = (viewport.x + point.x * viewport.z) * this._width;
    out.y = (viewport.y + point.y * viewport.w) * this._height;
    return out;
  }
